  ...
  <div v-loading="loading"
    ...
    <div class="tacData">
      ...
      <div class="h-chart" v-show="dashboardType === '1'">
        <h3 class="h3">按检修班组分布</h3>
        <el-button type="primary" @click="jumpDept('tt')">查看检修分项点汇总对比</el-button>
        <div class="table h-chart">
          <table>
            <thead>
            <tr>
              <th rowspan="2">排名</th>
              <th rowspan="2">检修班组</th>
        ...
        <div class="h-chart">
          <highcharts :options="vodOmVideoTtPie"></highcharts>
        </div>
        <div class="h-chart">
          <highcharts :options="vodOmVideoTtRate"></highcharts>
        </div>
        <div class="h-chart">
          <highcharts :options="vodOmVideoTtRateTrend"></highcharts>
        </div>
      </div>
      <div class="h-chart" v-show="dashboardType === '2'">
        ...
        <div class="h-chart">
          <highcharts :options="vodOmVideoTypePie"></highcharts>
        </div>
        <div class="h-chart">
          <highcharts :options="vodOmVideoTypeRate"></highcharts>
        </div>
        <div class="h-chart">
          <highcharts :options="vodOmVideoTypeRateTrend"></highcharts>
        </div>
      </div>
    </div>
    <div v-if="loginPopup">
    ...
    methods: {
      ...
      getPersonDashboard () {
        ...
        this.axios({
          ...
          url: '/api/data-analysis/om-video'
        }).then((response) => {
          self.loading = false
          ...
          if (data.type === 1) {
            ...
            self.ttCodes = data.result.omTeamCharts.codes
            // 图表数据
            self.vodOmVideoTtPie.series[0].data = data.result.omTeamCharts.chart3
            self.vodOmVideoTtRate.xAxis.categories = data.result.omTeamCharts.categories
            self.vodOmVideoTtRate.series = data.result.omTeamCharts.chart2
            self.vodOmVideoTtRateTrend.xAxis.categories = data.result.omTeamCharts.categories
            self.vodOmVideoTtRateTrend.series[0].data = data.result.omTeamCharts.chart1
            self.vodOmVideoTypePie.series[0].data = data.result.typeCharts.chart3
            self.vodOmVideoTypeRate.xAxis.categories = data.result.typeCharts.categories
            self.vodOmVideoTypeRate.series = data.result.typeCharts.chart2
            self.vodOmVideoTypeRateTrend.xAxis.categories = data.result.typeCharts.categories
            self.vodOmVideoTypeRateTrend.series[0].data = data.result.typeCharts.chart1
            if (self.date.timeUnit === 'W') {
              self.vodOmVideoTtRateTrend.series[0].name = '较前一周环比'
              self.vodOmVideoTypeRateTrend.series[0].name = '较前一周环比'
            } else if (self.date.timeUnit === 'M') {
              self.vodOmVideoTtRateTrend.series[0].name = '较前一月环比'
              self.vodOmVideoTypeRateTrend.series[0].name = '较前一月环比'
            }
            // 表格数据
            self.ttList = data.result.omTeamCharts.table
